...
impl StatusBar {
    ...
    pub fn render(
        ...
        if let Some(msg) = message {
            let mut display_msg = msg.to_string();

            // Pad or truncate to full width
            if display_msg.len() < term_width as usize {
                display_msg.push_str(&" ".repeat(term_width as usize - display_msg.len()));
            } else {
                display_msg.truncate(term_width as usize);
            }

            terminal.set_fg(Color::Yellow)?;
            terminal.print(&display_msg)?;
            terminal.reset_color()?;
        } else {
            // Build normal status line content
